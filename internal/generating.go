...
func generateUserID(collUsers *mongo.Collection, collVerify *mongo.Collection) (string, error) {
	...
	for i := 0; i < 20; {
		...
		defer cancel()
		count_users, err = collUsers.CountDocuments(ctx, bson.D{{Key: "token", Value: userID}})
		if err != nil {
			return "", err
		}
		// TODO: Stop after the first found
		...
		ctx,cancel = context.WithTimeout(context.Background(), time.Millisecond * 100)
		defer cancel()
		count_verify, err = collVerify.CountDocuments(ctx, bson.D{{Key: "token", Value: userID}})
		if err != nil {
			return "", err
		}
...
func generateSessionToken(collSession *mongo.Collection) (string, error) {
	...
	for i := 0; i < 20; {
		...
		ctx,cancel := context.WithTimeout(context.Background(), time.Millisecond * 100)
		defer cancel()
		count, err = collSession.CountDocuments(ctx, bson.D{{Key: "token", Value: token}})
